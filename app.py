...
def setup():
    if not os.path.exists(config.model_folder):
        if (
            input(
                f"Model folder {config.model_folder} does not exist. Create it? (y/n) "
            ).lower()
            == "y"
        ):
    ...
    for model in config.models:
        ...
        if config.models[model]["type"] == "local":
            if config.models[model]["filename"] not in current:
                if (
                    input(
                        f"Model {model} not found in {config.model_folder}. Would you like to download it? (y/n) "
                    ).lower()
                    == "y"
                ):
...
# TODO: make a version for Windows/Linux
        ...
        if sender.state:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    rumps.alert(
                        "Error",
                        f"Failed to send selection. Server responded with: {response.status_code}.",
                    )
